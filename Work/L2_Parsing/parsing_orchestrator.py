...
def orchestrate_parsing(
    ...
    atom_a_func: Callable[[str, str], List[Dict]],
    ...
    输入:
        input_files: 根文件路径列表
        atom_a_func: Atom A函数 (text, source_file -> List[ParsedItem])
...
def _parse_file_recursive(
    ...
    # Call Atom A with text and source_file (Plan_v2.txt Section 1)
    try:
        parsed_items = atom_a_func(content, abs_path)
    except Exception:
        # Atom A failed, skip this file
        return
    
    # Add to results (source_file already set by Atom A)
