"""
运行Check_10_0_0_00_generated.py生成4种Type的reports
对比CodeGen vs Golden (aggressive) 输出
"""
import sys
...
def run_type(config_file, type_name, type_suffix):
    ...
    custom_rpt = original_rpt.parent / f"{original_rpt.stem}_CodeGen_{type_suffix}.rpt"
    checker.rpt_path = custom_rpt
    
    # Also update log path for consistency
    original_log = checker.log_path
    custom_log = original_log.parent / f"{original_log.stem}_CodeGen_{type_suffix}.log"
