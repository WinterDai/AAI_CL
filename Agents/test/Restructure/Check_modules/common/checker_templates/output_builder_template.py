...
class OutputBuilderMixin:
        ...
        **API v2.0 CHANGES:**
        ...
        if found_items:
            for item_name in sorted(found_items.keys()):
                ...
                # Handle __INFO__ prefix (pure display items that don't count toward value)
                if item_name.startswith('__INFO__'):
                    display_name = item_name[8:]  # Remove __INFO__ prefix
                
                if name_extractor:
                    display_name = name_extractor(display_name, metadata)
    ...
    def build_result_groups(
        ...
        **API v2.0 CHANGES:**
        ...
        if found_items:
            found_item_names = []
            for item_name in sorted(found_items.keys()):
                # Handle __INFO__ prefix (pure display items)
                display_name = item_name
                if item_name.startswith('__INFO__'):
                    display_name = item_name[8:]  # Remove __INFO__ prefix
    ...
    # =========================================================================
    # TEMPLATE METHOD PATTERN - Type Execution Simplification (v2.1.0)
    # =========================================================================
    
    def execute_boolean_check(self, has_waiver: bool = False) -> CheckResult:
        """
        Template Method for Type 1/4 (Boolean Check).
        
        LLM只需实现：
        1. _parse_input_files() → 返回 Dict[str, Any] 包含 'items' 列表
        2. FOUND_DESC, MISSING_DESC 等常量
        ...
        for item in items:
            if isinstance(item, dict):
                ...
                for key, value in item.items():
                    if key not in ['name', 'line_number', 'file_path']:
                        found_items[name][key] = value
            else:
                found_items[str(item)] = {'line_number': 0, 'file_path': 'N/A'}
        
        # 3. 确定violations（Boolean check：有item就pass，无item就fail）
        missing_items = []
        if not found_items:
            # 从常量推断检查目标名称
            target_name = getattr(self, 'item_desc', 'Required item')
            missing_items.append(target_name)
        
        # 4. 处理waiver（如果has_waiver=True）
        if has_waiver:
            waivers = self.get_waivers()
            waive_items_raw = waivers.get('waive_items', [])
            waive_dict = self.parse_waive_items(waive_items_raw)
            
            # 分类waived vs unwaived
            waived_items = []
            unwaived_items = []
            for violation in missing_items:
            ...
            return self.build_complete_output(
                ...
                waive_dict=waive_dict,
                unused_waivers=unused_waivers,
                found_desc=getattr(self, 'FOUND_DESC', 'Items found'),
                missing_desc=getattr(self, 'MISSING_DESC', 'Items missing'),
                found_reason=getattr(self, 'FOUND_REASON', 'Item successfully found'),
                missing_reason=getattr(self, 'MISSING_REASON', 'Required item not found'),
                waived_desc=getattr(self, 'WAIVED_DESC', 'Waived items'),
                waived_base_reason=getattr(self, 'WAIVED_BASE_REASON', 'Item waived'),
                unused_waiver_reason=getattr(self, 'UNUSED_REASON', 'Waiver not used')
            )
        else:
            # Type 1: 简单输出（无waiver）
            return self.build_complete_output(
    ...
    def execute_value_check(self, has_waiver: bool = False) -> CheckResult:
        """
        Template Method for Type 2/3 (Value Check with pattern_items).
        
        LLM只需实现：
        1. _parse_input_files() → 返回 Dict[str, Any] 包含 'items' 列表
        2. 可选: _determine_extra_items() → 返回extra_items dict（如果需要extra逻辑）
        3. FOUND_DESC, MISSING_DESC 等常量
        ...
        # 2. 获取pattern_items（requirements配置）
        ...
        # 4. 确定violations（pattern_items中缺失的项）
        found_names = set(found_items.keys())
        violations = [p for p in pattern_items if p not in found_names]
        
        # 5. 处理extra_items（可选，如果LLM定义了_determine_extra_items）
        extra_items = {}
        if hasattr(self, '_determine_extra_items'):
            extra_items = self._determine_extra_items(data, pattern_items)
        
        # 6. 处理waiver（如果has_waiver=True）
        if has_waiver:
            waivers = self.get_waivers()
            waive_items_raw = waivers.get('waive_items', [])
            waive_dict = self.parse_waive_items(waive_items_raw)
            ...
            return self.build_complete_output(
                ...
                found_desc=getattr(self, 'FOUND_DESC', 'Items found'),
                missing_desc=getattr(self, 'MISSING_DESC', 'Items missing'),
                found_reason=getattr(self, 'FOUND_REASON', 'Pattern matched'),
                missing_reason=getattr(self, 'MISSING_REASON', 'Pattern not matched'),
                waived_desc=getattr(self, 'WAIVED_DESC', 'Waived items'),
                waived_base_reason=getattr(self, 'WAIVED_BASE_REASON', 'Item waived'),
                unused_waiver_reason=getattr(self, 'UNUSED_REASON', 'Waiver not used'),
                extra_reason=getattr(self, 'EXTRA_REASON', 'Extra item found') if extra_items else None,
                extra_desc=getattr(self, 'EXTRA_DESC', 'Extra items') if extra_items else None
            )
        else:
            # Type 2: 简单输出（无waiver）
            return self.build_complete_output(
    ...
    # =========================================================================
    # Aggressive Refactoring: Generic Type Executors
    # =========================================================================
    ...
    def execute_boolean_check(
        ...
        **output_params
        ...
        """
        通用Boolean检查执行器（Type 1/4）
        
        这是激进重构方案的核心方法，将Type 1/4的共同执行流程提取到Framework。
        LLM只需要生成parse_data_func中的业务逻辑（30-50行），不需要关心：
        - found/missing分类逻辑
        - waiver处理逻辑
        - build_complete_output调用
        
        Args:
            parse_data_func: LLM生成的业务逻辑函数，返回(found_items, missing_items, extra_items)
                found_items: Dict[name, metadata] - 通过检查的项
                missing_items: Dict[name, metadata] - 未通过检查的项
                extra_items: Dict[name, metadata] - 额外的项（如SPEF skip）
            has_waiver: 是否支持waiver.value (Type 1=False, Type 4=True)
            **output_params: 传递给build_complete_output的额外参数
                支持: found_reason, missing_reason, found_desc, missing_desc等
        
        Returns:
            CheckResult: 完整的检查结果
        
        Example (Type 1):
            def _execute_type1(self, netlist_info, spef_info, errors):
                def parse_data():
                    found_items = {}
                    missing_items = {}
                    extra_items = {}
                    ...
                    if netlist_info.get('path'):
                        found_items['Netlist File'] = {
                            'line_number': netlist_info.get('line_number', 0),
                            'file_path': netlist_info.get('path', 'N/A')
                        }
                    else:
                        missing_items['Netlist File'] = {'line_number': 0, 'file_path': 'N/A'}
                    
                    if spef_info.get('status') == 'Skipped':
                        extra_items['SPEF Reading was skipped'] = {
                            'line_number': spef_info.get('line_number', 0),
                            'file_path': spef_info.get('path', 'N/A'),
                            'reason': spef_info.get('skip_reason', 'SPEF skipped')
                        }
                ...
                return self.execute_boolean_check(
                    parse_data,
                    has_waiver=False,
                    found_reason="File exists",
                    missing_reason="File not found",
                    extra_reason="Design has no spef file"
                )
        """
        # 1. 调用LLM生成的业务逻辑
        found_items, missing_items, extra_items = parse_data_func()
        
        # 2. 处理waiver（如果has_waiver=True）
        if has_waiver:
            waivers = self.get_waivers()
            waive_items_raw = waivers.get('waive_items', [])
            waive_dict = self.parse_waive_items(waive_items_raw)
            ...
            waived_items = {}
            unwaived_items = {}
            for name, metadata in missing_items.items():
                if self.match_waiver_entry(name, waive_dict):
                    waived_items[name] = metadata
                else:
                    unwaived_items[name] = metadata
            
            # 计算unused waivers
            used_names = set(waived_items.keys())
            unused_waivers_list = [name for name in waive_dict.keys() if name not in used_names]
            unused_waivers = {name: {} for name in unused_waivers_list}
            
            # 3. 调用build_complete_output（Type 4）
            return self.build_complete_output(
                found_items=found_items,
                missing_items=unwaived_items,
                waived_items=waived_items,
                ...
                unused_waivers=unused_waivers,
                extra_items=extra_items if extra_items else None,
                value='yes' if not unwaived_items else 'N/A',
            ...
            # 3. 调用build_complete_output（Type 1）
            return self.build_complete_output(
                found_items=found_items,
                missing_items=missing_items,
                extra_items=extra_items if extra_items else None,
                value='yes' if not missing_items else 'N/A',
                has_pattern_items=False,
    ...
    def execute_value_check(
        ...
        **output_params
        ...
        """
        通用Value检查执行器（Type 2/3）
        
        这是激进重构方案的核心方法，将Type 2/3的共同执行流程提取到Framework。
        LLM只需要生成parse_data_func中的业务逻辑（40-60行），不需要关心：
        - pattern_items匹配逻辑
        - found/missing分类逻辑
        - waiver处理逻辑
        - build_complete_output调用
        ...
        Args:
            parse_data_func: LLM生成的业务逻辑函数，返回(found_items, missing_items, extra_items)
                found_items: Dict[pattern, metadata] - 匹配pattern_items的项
                missing_items: Dict[pattern, metadata] - 未匹配pattern_items的项
                extra_items: Dict[name, metadata] - 不在pattern_items中的额外项
            has_waiver: 是否支持waiver.value (Type 2=False, Type 3=True)
            **output_params: 传递给build_complete_output的额外参数
        ...
        Example (Type 2):
            def _execute_type2(self, netlist_info, spef_info, errors):
                def parse_data():
                    # 获取pattern_items
                    requirements = self.get_requirements()
                    ...
                    if netlist_info.get('version'):
                        all_content.append(f"Version: {netlist_info['version']}")
        ...
        # 1.5 处理info_items（不影响value的纯展示INFO项）
        if info_items:
            # 将info_items合并到found_items中，它们会显示为INFO detail但不计入value
            # value计算在后面会排除这些__INFO__前缀的项
            for name, metadata in info_items.items():
                found_items[f"__INFO__{name}"] = metadata
        ...
        if has_waiver:
            ...
            # 3. 调用build_complete_output（Type 2）
            return self.build_complete_output(
                found_items=found_items,
                missing_items=missing_items,
                extra_items=extra_items if extra_items else None,
