################################################################################
# Script Name: IMP-5-0-0-17.py
#
# Purpose:
#   Confirm CPF/UPF file has passed Conformal LP quality checks post-DFT.
#   Parse verify_power_structure_post_dft.rpt for Severity: Error issues.
#
# Logic:
...
class PowerStructurePostDFTChecker(BaseChecker):
    """
    Unified checker for power structure verification that auto-detects and handles all 4 checker types.
    
    Check Target: Confirm CPF/UPF file has passed Conformal LP quality checks post-DFT
    ...
    def __init__(self):
        super().__init__(
            ...
            item_desc="Confirm CPF/UPF file  has passed Conformal LP quality checks against post DFT insertion synthesis netlist?"
        )
    ...
    def execute_check(self) -> CheckResult:
        ...
        """
        try:
            valid_files, missing_files = self._get_input_files()
            ...
            all_errors = self._parse_power_structure_report(valid_files)
            
            if not all_errors:
                # No severity errors found - report detailed PASS info
                pass_details: List[DetailItem] = []
                if valid_files:
                    for report_path in valid_files:
                        pass_details.append(DetailItem(
                            severity=Severity.INFO,
                            name=f"No Severity: Error issues in {report_path.name}",
                            line_number="0",
                            file_path=str(report_path),
                            reason=f"All Severity: Error checks passed in {report_path}"
                        ))
                else:
                    pass_details.append(DetailItem(
                        severity=Severity.INFO,
                        name="No input report provided",
                        line_number="0",
                        file_path="N/A",
                        reason="No report file configured; skipped Severity: Error scan"
                    ))
                
                info_groups = {
                    "INFO01": {
                        "description": "CPF/UPF file has passed Conformal LP quality checks",
                        "items": [detail.name for detail in pass_details]
                    }
                }
    ...
    def _get_input_files(self) -> Tuple[List[Path], List[str]]:
    ...
    def _parse_power_structure_report(self, input_files: List[Path]) -> List[str]:
        """
    ...
    def _execute_type2(self, all_errors: List[str], 
        ...
        for error_name, pattern in errors_matched:
            ...
            # Add [WAIVED_AS_INFO] tag when waiver=0
            if str(waiver_value) == '0':
                reason = f"{issue_desc}[WAIVED_AS_INFO]"
                severity = Severity.INFO
            else:
                reason = issue_desc
...
def run():
    """Main entry point."""
    checker = PowerStructurePostDFTChecker()
