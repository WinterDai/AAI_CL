...
"""
    ...
    print(f"✅ 已保存 {len(reviews)} 个执行日志和报告")
    ...
    # ========================================================================
    # Step 6: 执行 checker.py 并收集真实输出
    # ========================================================================
    print_section("Step 6: 执行 checker.py (收集真实输出)")
    ...
    try:
        ...
        if exec_result.success:
            ...
            if exec_result.log_content:
                log_path = output_mgr.save_checker_log(exec_result.log_content)
                print(f"⚠️ 已保存失败时的 log: {log_path}")
                
    except ImportError as e:
        print(f"⚠️ 无法导入 CheckerExecutor: {e}")
        print("   跳过 checker.py 执行步骤")
    except Exception as e:
        print(f"❌ checker.py 执行过程出错: {e}")
        import traceback
        traceback.print_exc()
    ...
    # Step 7: 生成完整 README
    # ========================================================================
    print_section("Step 7: 生成 README.md")
