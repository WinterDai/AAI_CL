...
"""
    ...
    # ========================================================================
    # Step 6: Execute checker.py for each TestCase
    # ========================================================================
    print_section("Step 6: Execute checker.py for Each TestCase")
    ...
    try:
        ...
        # Get base item config
        base_config = item_config.to_dict() if item_config else {}
        
        # Execute checker.py once with original config (baseline)
        print(f"\n[RUN] Executing baseline (original item.yaml)...")
        exec_result = executor.execute_checker(generated_code, ITEM_ID)
        
        if exec_result.success:
            print(f"[OK] Baseline executed (is_pass={exec_result.is_pass})")
            if exec_result.log_content:
                output_mgr.save_checker_log(exec_result.log_content)
            if exec_result.report_content:
                output_mgr.save_checker_report(exec_result.report_content)
            if exec_result.check_result:
                output_mgr.save_checker_pkl(exec_result.check_result)
        else:
            print(f"[WARN] Baseline execution failed: {exec_result.error_message}")
        
        # Execute checker.py for each test case
        print(f"\n[INFO] Executing checker.py for {len(test_cases)} test cases...")
        ...
        for tc in test_cases:
            ...
            tc_type = tc.get("type_id", tc.get("type", 1))
            tc_direction = tc.get("direction", "positive")
            config_override = tc.get("config_override", {})
            
            # Load test case yaml if exists
            tc_yaml_path = output_mgr.test_cases_dir / f"{ITEM_ID}_type{tc_type}_{tc_direction}.yaml"
            tc_config = {}
            if tc_yaml_path.exists():
                with open(tc_yaml_path, 'r', encoding='utf-8') as f:
                    tc_config = yaml.safe_load(f) or {}
            else:
                tc_config = {
            ...
            # Show config override details
            override = tc_config.get("config_override", {})
            print(f"\n[RUN] {tc_id}: Type {tc_type} ({tc_direction})")
            if override:
                print(f"   config_override: {override}")
            
            # Execute with test case config
            exec_result = executor.execute_with_test_case(
                generated_code, ITEM_ID, base_config, tc_config
            )
            ...
            tc_result = {
                ...
                "is_pass": exec_result.is_pass,
            }
            
            if exec_result.success:
                print(f"   [OK] is_pass={exec_result.is_pass}")
                
                # Save outputs with tc_id prefix
                if exec_result.log_content:
                    log_path = output_mgr.save_checker_log(exec_result.log_content, tc_id)
                    tc_result["log_path"] = str(log_path)
                    
                if exec_result.report_content:
                    report_path = output_mgr.save_checker_report(exec_result.report_content, tc_id)
                    tc_result["report_path"] = str(report_path)
                    
                if exec_result.check_result:
                    pkl_path = output_mgr.save_checker_pkl(exec_result.check_result, tc_id)
                    tc_result["pkl_path"] = str(pkl_path)
            else:
                print(f"   [WARN] Execution failed: {exec_result.error_message or exec_result.stderr[:100] if exec_result.stderr else 'unknown'}")
                tc_result["error"] = exec_result.error_message or exec_result.stderr
            
            tc_results.append(tc_result)
        
        # Summary
        success_count = sum(1 for r in tc_results if r.get("success"))
        print(f"\n[INFO] TestCase execution summary: {success_count}/{len(tc_results)} succeeded")
