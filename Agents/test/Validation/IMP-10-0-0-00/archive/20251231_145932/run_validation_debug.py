...
async def run_validation_debug():
    ...
    # ========================================================================
    # Step 3: 运行 CodeGen 获取 checker 代码
    # ========================================================================
    print_section("Step 3: 运行 CodeGenAgent (或加载已有代码)")
    
    # 检查是否有已生成的代码
    codegen_output_dir = _AGENT_DIR / "test" / "CodeGen" / ITEM_ID
    existing_checker = codegen_output_dir / f"Check_{ITEM_ID.replace('-', '_').replace('IMP_', '')}.py"
    
    generated_code = None
    item_spec = None
    
    if existing_checker.exists():
        print(f"发现已有 CodeGen 输出: {existing_checker}")
        with open(existing_checker, 'r', encoding='utf-8') as f:
            generated_code = f.read()
        print(f"✅ 已加载 checker 代码 ({len(generated_code)} chars)")
        
        # 尝试加载 item_spec
        codegen_debug = codegen_output_dir / "codegen_debug.json"
        if codegen_debug.exists():
            with open(codegen_debug, 'r', encoding='utf-8') as f:
                debug_data = json.load(f)
                item_spec = debug_data.get("item_spec", {})
    else:
        print("未找到已有 CodeGen 输出，将调用 CodeGenAgent...")
        ...
        # 运行 CodeGenAgent
        from agents.code_generation.agent import CodeGenAgent
        
        codegen = CodeGenAgent(debug_mode=True)
        ...
        if codegen_result.status == "success" and codegen_result.result:
            ...
            print(f"❌ CodeGen 失败: {codegen_result.errors}")
            return False
