...
- Describe pattern matching capability for each validation item
- Specify pattern_items correspondence mapping for all items
...
<!-- TODO: For each validation item, describe its pattern matching support -->

Each validation item supports two checking modes (determined by runtime configuration):
- **Existence check mode** (when requirements.value=N/A): Verify item presence only
- **Pattern matching mode** (when requirements.value>0): Match against pattern_items

Describe pattern matching capability for each item:

- **Item 1: {TODO: Item name}**
  - Pattern matching support: {TODO: Yes/No}
  - If supported, matchable content: {TODO: Describe what can be matched, e.g., "tool name and version number"}
  - Expected pattern format: {TODO: e.g., "tool_name version", "YYYY-MM-DD", etc.}
  - Pattern correspondence: pattern_items[0] (if pattern matching is enabled at runtime)

- **Item 2: {TODO: Item name}**
  - Pattern matching support: {TODO: Yes/No}
  - If supported, matchable content: {TODO: Describe what can be matched}
  - Expected pattern format: {TODO: Format description}
  - Pattern correspondence: pattern_items[1] (if pattern matching is enabled at runtime)

<!-- Add more items as needed (typically 2-6 items) -->

**Pattern Correspondence Summary**:
<!-- This mapping is used when requirements.value>0 and pattern_items is provided -->
- pattern_items[0] → Item 1 (if pattern matching supported)
- pattern_items[1] → Item 2 (if pattern matching supported)
<!-- Add mappings for all items that support pattern matching -->

**Note**: 
- Each pattern_item can use `|` to separate multiple alternative patterns (OR relationship)
- Items without pattern matching support are skipped in pattern_items list
- Actual checking mode is determined by runtime configuration (requirements.value), not by ItemSpec
