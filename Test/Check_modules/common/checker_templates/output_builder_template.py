...
class OutputBuilderMixin:
    ...
    # =========================================================================
    # Three-Layer Architecture API (v2.0) - Lambda-based Wrappers
    # =========================================================================
    
    def execute_boolean_check(
        self,
        parse_data_func: Callable[[], Dict[str, Any]],
        ...
        """
        Execute Boolean Check (Type 1 / Type 4) with lambda-based data parsing.
        
        Three-Layer API wrapper that handles:
        - Layer 1: Data parsing (via parse_data_func callback)
        - Layer 2: Output building (details + groups)
        - Layer 3: CheckResult creation
        
        Type 1 (No waiver, has_waiver=False):
            found_items = items matching condition
            missing_items = items NOT matching condition (violations)
        
        Type 4 (Has waiver, has_waiver=True):
            found_items = items matching condition
            missing_items = unwaived violations
            waived_items = waived violations
            unused_waivers = waivers not matched
        
        Args:
            parse_data_func: Lambda/function that returns categorized items dict
                Expected return format: {
                    'found_items': Dict[str, Dict],
                    'missing_items': Dict[str, Dict],
                    'waived_items': Dict[str, Dict],  # Type 4 only
                    'unused_waivers': Dict[str, Dict],  # Type 4 only
                    'waive_dict': Dict[str, str],  # Type 4 only
                    'found_reason': str,
                    'missing_reason': str,
                    'waived_base_reason': str,  # Type 4 only
                }
            has_waiver: True for Type 4, False for Type 1
            found_desc: Description for found items group
            missing_desc: Description for missing items group
            extra_desc: Description for extra items (if any)
            waived_desc: Description for waived items group (Type 4)
            unused_desc: Description for unused waivers group (Type 4)
            name_extractor: Optional callable to extract display name from metadata
        
        Returns:
            CheckResult with categorized items
        
        Example (Type 1 - Simple Boolean):
            def parse_data():
                # Your parsing logic here
                return {
                    'found_items': {"group1": {"line_number": 10, "file_path": "report.txt"}},
                    'missing_items': {"group2": {"line_number": 20, "file_path": "report.txt"}},
                    'found_reason': "Group is clean",
                    'missing_reason': "Group has violations"
                }
            ...
            return self.execute_boolean_check(
                ...
                has_waiver=False,
                found_desc="Clean groups",
                missing_desc="Violated groups"
            )
        
        Example (Type 4 - Boolean with Waivers):
            def parse_data():
                return {
                    'found_items': found_dict,
                    'missing_items': unwaived_dict,
                    'waived_items': waived_dict,
                    'unused_waivers': unused_dict,
                    'waive_dict': waive_reasons,
                    'found_reason': "Group is clean",
                    'missing_reason': "Group has violations",
                    'waived_base_reason': "Group has violations"
                }
        ...
        """
        # Call parse_data_func to get categorized items
        parsed_result = parse_data_func()
        
        # Handle both tuple and dict return values
        if isinstance(parsed_result, tuple):
            # Tuple format: (found_items, missing_items, extra_items)
            # For Type 1/4 Boolean checks, extra_items is not used
            found_items = parsed_result[0] if len(parsed_result) > 0 else {}
            missing_items = parsed_result[1] if len(parsed_result) > 1 else {}
            # Set default reasons
            found_reason = "Item found"
            missing_reason = "Item not found"
            waived_items = {}
            unused_waivers = {}
            waive_dict = {}
            waived_base_reason = "Item not found"
        else:
            # Dict format: full structured response
            found_items = parsed_result.get('found_items', {})
            missing_items = parsed_result.get('missing_items', {})
            waived_items = parsed_result.get('waived_items', {})
            unused_waivers = parsed_result.get('unused_waivers', {})
            waive_dict = parsed_result.get('waive_dict', {})
            # Extract reasons
            found_reason = parsed_result.get('found_reason', "Item found")
            missing_reason = parsed_result.get('missing_reason', "Item not found")
            waived_base_reason = parsed_result.get('waived_base_reason', "Item not found")
        
        # Build output using unified API
        return self.build_complete_output(
            found_items=found_items,
            missing_items=missing_items,
            waived_items=waived_items if has_waiver else None,
            unused_waivers=unused_waivers if has_waiver else None,
            waive_dict=waive_dict if has_waiver else None,
            value=len(found_items),
            has_pattern_items=has_waiver,  # Type 4 has pattern_items
            has_waiver_value=has_waiver,
            name_extractor=name_extractor,
            found_reason=found_reason,
            missing_reason=missing_reason,
            waived_base_reason=waived_base_reason,
            waived_tag="[WAIVER]",  # Type 4 uses [WAIVER]
            found_desc=found_desc,
            missing_desc=missing_desc,
            waived_desc=waived_desc,
            unused_desc=unused_desc
    ...
    def execute_value_check(
        ...
        parse_data_func: Callable[[], Dict[str, Any]],
        has_waiver: bool = False,
        info_items: Optional[List[str]] = None,  # NEW: Support waiver=0 mode
        extra_severity: Optional[Severity] = None,  # NEW: Allow custom extra items severity
        found_desc: str = "Items found",
        missing_desc: str = "Items not found",
        extra_desc: str = "Unexpected items",
        waived_desc: str = "Items waived",
        ...
        """
        Execute Value Check (Type 2 / Type 3) with lambda-based data parsing.
        ...
        Type 2 (No waiver, has_waiver=False):
            found_items = items in pattern_items AND found
            missing_items = items in pattern_items but NOT found
            extra_items = items found but NOT in pattern_items
        
        Type 3 (Has waiver, has_waiver=True):
            found_items = items in pattern_items AND found
            missing_items = unwaived missing
            waived_items = waived missing
            extra_items = items found but NOT in pattern_items
            unused_waivers = waivers not matched
        ...
        Args:
            parse_data_func: Lambda/function that returns categorized items dict
                Expected return format: {
                    'found_items': Dict[str, Dict],
                    'missing_items': Dict[str, Dict],
                    'extra_items': Dict[str, Dict],  # Optional
                    'waived_items': Dict[str, Dict],  # Type 3 only
                    'unused_waivers': Dict[str, Dict],  # Type 3 only
                    'waive_dict': Dict[str, str],  # Type 3 only
                    'found_reason': str,
                    'missing_reason': str,
                    'extra_reason': str,  # Optional
                    'waived_base_reason': str,  # Type 3 only
                }
            has_waiver: True for Type 3, False for Type 2
            found_desc: Description for found items group
            missing_desc: Description for missing items group
            extra_desc: Description for extra items group
            waived_desc: Description for waived items group (Type 3)
            unused_desc: Description for unused waivers group (Type 3)
            name_extractor: Optional callable to extract display name from metadata
        
        Returns:
            CheckResult with categorized items
        
        Example (Type 2 - Pattern Matching):
            def parse_data():
                return {
                    'found_items': {"clock1": {"line_number": 10, "file_path": "report.txt"}},
                    'missing_items': {"clock3": {"line_number": 0, "file_path": "N/A"}},
                    'extra_items': {"unexpected": {"line_number": 30, "file_path": "report.txt"}},
                    'found_reason': "Clock configured correctly",
                    'missing_reason': "Clock configuration missing",
                    'extra_reason': "Unexpected clock found"
                }
            
            return self.execute_value_check(
                parse_data_func=parse_data,
                has_waiver=False,
                found_desc="Configured clocks",
                missing_desc="Missing clocks",
                extra_desc="Unexpected clocks"
            )
        
        Example (Type 3 - Pattern + Waivers):
            def parse_data():
                return {
                    'found_items': found_dict,
                    'missing_items': unwaived_dict,
                    ...
                    'missing_reason': "Clock configuration missing",
                    'extra_reason': "Unexpected clock found",
                    'waived_base_reason': "Clock configuration missing"
                }
            ...
            return self.execute_value_check(
                ...
                missing_desc="Missing clocks",
                extra_desc="Unexpected clocks",
                waived_desc="Waived missing clocks",
                unused_desc="Unused waivers"
            )
        """
        # Call parse_data_func to get categorized items
        ...
        if isinstance(parsed_result, tuple):
            ...
            found_items = parsed_result[0] if len(parsed_result) > 0 else {}
            missing_items = parsed_result[1] if len(parsed_result) > 1 else {}
            extra_items = parsed_result[2] if len(parsed_result) > 2 else {}
            # Set default reasons
            found_reason = "Item found"
            missing_reason = "Item not found"
            extra_reason = "Unexpected item found"
            waived_items = {}
            unused_waivers = {}
            waive_dict = {}
            waived_base_reason = "Item not found"
            ...
            found_items = parsed_result.get('found_items', {})
            missing_items = parsed_result.get('missing_items', {})
            extra_items = parsed_result.get('extra_items', {})
            waived_items = parsed_result.get('waived_items', {})
            unused_waivers = parsed_result.get('unused_waivers', {})
            waive_dict = parsed_result.get('waive_dict', {})
            # Extract reasons
            found_reason = parsed_result.get('found_reason', "Item found")
            missing_reason = parsed_result.get('missing_reason', "Item not found")
            extra_reason = parsed_result.get('extra_reason', "Unexpected item found")
            waived_base_reason = parsed_result.get('waived_base_reason', "Item not found")
        ...
        return self.build_complete_output(
            ...
            waive_dict=waive_dict if has_waiver else None,
            value=len(found_items),
            has_pattern_items=True,  # Type 2/3 always has pattern_items
            has_waiver_value=has_waiver,
            name_extractor=name_extractor,
            found_reason=found_reason,
            missing_reason=missing_reason,
            extra_reason=extra_reason,
            extra_severity=extra_severity if extra_severity is not None else Severity.WARN,  # Use provided or default WARN
            waived_base_reason=waived_base_reason,
            waived_tag="[WAIVER]",  # Type 3 uses [WAIVER]
            found_desc=found_desc,
            missing_desc=missing_desc,
