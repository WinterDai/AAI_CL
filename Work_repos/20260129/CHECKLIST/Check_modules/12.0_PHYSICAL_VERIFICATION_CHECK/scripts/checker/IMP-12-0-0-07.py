################################################################################
# Script Name: IMP-12-0-0-07.py
#
# Purpose:
#   Confirm the ANT rule deck setting is correct.
#
# Logic:
#   - Parse do_pvs_ANT_pvl.log to extract variable definitions and configuration warnings
#   - Parse do_cmd_3star_ANT_sourceme to extract #DEFINE switches and LAYOUT settings
#   - Build configuration registry with PVL log values taking precedence over sourceme
#   - Validate actual settings against pattern_items (expected configuration)
...
class Check_12_0_0_07(InputFileParserMixin, OutputBuilderMixin, WaiverHandlerMixin, BaseChecker):
    """
    ...
    def _parse_input_files(self) -> Dict[str, Any]:
        """
        Parse input files to extract ANT rule deck configuration settings.
        
        Parses two files in priority order:
        1. do_cmd_3star_ANT_sourceme (lower priority - intended configuration)
        2. do_pvs_ANT_pvl.log (higher priority - actual tool behavior, overwrites sourceme)
        
        Builds configuration registry with actual settings from PVL log taking precedence.
        
        Returns:
            Dict with parsed data:
            - 'items': List[Dict] - Configuration settings with name, value, source, enabled state
            - 'metadata': Dict - File metadata (rule deck path, layout system, primary cell)
        ...
        # 3. Parse each input file for ANT rule deck configuration
                ...
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    for line_num, line in enumerate(f, 1):
                        ...
                        if match_define:
                            ...
                            define_name = match_define.group(2)
    ...
    def _execute_type1(self) -> CheckResult:
        ...
        Validates ANT rule deck configuration by checking:
        1. Critical settings exist in configuration files
        2. Settings have correct values (disabled/enabled as expected)
        3. No conflicting settings between PVL log and sourceme
        
        Returns PASS if all validation checks succeed, FAIL otherwise.
        """
        violations = self._type1_core_logic()
        
        # Build found_items from clean configuration settings
    ...
    def _type1_core_logic(self) -> dict:
        ...
        Validates ANT rule deck configuration against expected settings:
        - ACC_DEBUG: Must be disabled (not enabled)
        - DEBUG_VIA: Must be disabled (not enabled)
        - FHDMIM: Must exist in configuration
        - Other critical settings as needed
        ...
        if errors:
            # If parsing failed, return violation indicating configuration not found
            return {
                'configuration_error': {
                    'line_number': 0,
                    'file_path': 'N/A',
                    'reason': f"Failed to parse ANT configuration files: {'; '.join(errors)}"
                }
            }
        
        config_registry = data.get('config_registry', {})
        
        # If no configuration found at all, this is a violation
        if not config_registry:
            return {
                'missing_configuration': {
                    'line_number': 0,
                    'file_path': 'N/A',
                    'reason': 'No ANT rule deck configuration found in input files'
                }
            }
        
        violations = {}
        
        # Define expected configuration rules
        # Rule 1: ACC_DEBUG should be disabled (not enabled)
        if 'ACC_DEBUG' in config_registry:
            acc_debug = config_registry['ACC_DEBUG']
            value = acc_debug.get('value', '').lower()
            if value == 'enabled' or value == '1' or value == 'true':
                violations['ACC_DEBUG'] = {
                    'line_number': acc_debug.get('line_number', 0),
                    'file_path': acc_debug.get('file_path', 'N/A'),
                    'reason': f"Required setting not found in configuration files or has incorrect value (expected: disabled, actual: {acc_debug.get('value', 'unknown')})"
                }
        else:
            # ACC_DEBUG setting missing - this might be acceptable, so we don't flag it as violation
            pass
        
        # Rule 2: DEBUG_VIA should be disabled (not enabled)
        if 'DEBUG_VIA' in config_registry:
            debug_via = config_registry['DEBUG_VIA']
            value = debug_via.get('value', '').lower()
            if value == 'enabled' or value == '1' or value == 'true':
                violations['DEBUG_VIA'] = {
                    'line_number': debug_via.get('line_number', 0),
                    'file_path': debug_via.get('file_path', 'N/A'),
                    'reason': f"Required setting not found in configuration files or has incorrect value (expected: disabled, actual: {debug_via.get('value', 'unknown')})"
                }
        
        # Rule 3: Check for any configuration warnings from PVL log
        warnings = data.get('metadata', {}).get('warnings', [])
        for warning in warnings:
            warning_key = f"warning_{warning.get('line', 0)}"
            violations[warning_key] = {
                'line_number': int(warning.get('line', 0)),
                'file_path': warning.get('file', 'N/A'),
                'reason': f"Configuration warning detected: {warning.get('reason', 'Unknown warning')}"
            }
    ...
    def _execute_type4(self) -> CheckResult:
        ...
        Same validation as Type 1, but allows waiving specific configuration violations:
        - Violations matching waive_items → waived (PASS)
        - Violations not matching waive_items → unwaived (FAIL)
        - Clean settings → found_items (PASS)
        ...
        # FIXED: Pass dicts directly, not list(dict.values())
        return self.build_complete_output(
            found_items=found_items,
            missing_items=missing_items,
            waived_items=waived_items,
