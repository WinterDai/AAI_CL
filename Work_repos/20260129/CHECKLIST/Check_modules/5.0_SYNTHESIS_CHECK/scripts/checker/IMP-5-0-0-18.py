################################################################################
# Script Name: IMP-5-0-0-18.py
#
# Purpose:
#   1. Document DFT Stuck-At ATCov% generated by the DFT flow.
#   2. Extract coverage percentage from ATPG stuckat log files.
#   3. Compare actual coverage against expected value from DATA_INTERFACE.yaml.
#
# Usage:
#   python IMP-5-0-0-18.py
#   (Normally invoked by module runner 5.0_SYNTHESIS_CHECK.py)
#
#
# Logic:
#   - Parse DFT ATPG log to extract Stuck-At coverage percentage
...
"""IMP-5-0-0-18: DFT Stuck-At Coverage Check

Refactored to use BaseChecker for all 4 types with automatic detection.
Extracts ATCov% from ATPG log and compares against requirements.
"""
...
ITEM_ID = "IMP-5-0-0-18"
ITEM_DESC = "Document DFT Stuck-At % meets or exceeds expected value?"


class DFTStuckatChecker(BaseChecker):
    """
    Check DFT Stuck-At coverage percentage against requirements.
    ...
    Extracts ATCov% from ATPG stuckat log file (2nd percentage value).
    """
    ...
    def _parse_atpg_log(self, log_path: Path) -> Optional[float]:
        """
        Extract ATCov% from ATPG stuckat log file.
        
        Format in log:
        INFO (TDA-220):   1110    834      0    99.21%   96.19%    2853       00:50.41
    ...
    def _execute_type1(self) -> 'CheckResult':
        ...
        if not input_files:
            return self.create_config_error("ATPG stuck-at log file not configured")
        ...
        if coverage is None:
            return create_check_result(
                ...
                details=[],
                basic_errors=["ATCov% not found in ATPG log file"],
        ...
        reason = f"DFT Stuck-At coverage: {coverage}%[WAIVED_AS_INFO]" if is_waiver_zero else f"DFT Stuck-At coverage: {coverage}%"
        ...
        info_groups = {
            "INFO01": {
                "description": "DFT Stuck-At Coverage",
    ...
    def _execute_type2(self) -> 'CheckResult':
        ...
        if not expected_items:
            ...
            details = [
                ...
                DetailItem(
                    ...
                    line_number=str(metadata['line']),
                    file_path=str(metadata['file']),
                    reason=f"Current DFT Stuck-At coverage: {coverage}%"
                )
            ]
        ...
        if is_waiver_zero:
            ...
            reason = f"DFT Stuck-At coverage: {coverage}% (threshold: {expected_threshold}%)[WAIVED_AS_INFO]"
            details = [DetailItem(
                severity=Severity.INFO,
                name=f"ATCov%:{coverage}%",
                line_number=str(metadata['line']),
            ...
            info_groups = {
                "INFO01": {
                    "description": "DFT Stuck-At Coverage (waiver=0 display mode)",
        ...
        if coverage >= expected_threshold:
            ...
            error_groups = {
                "ERROR01": {
                    "description": "DFT Stuck-At coverage below requirement",
                    "items": [f"ATCov%:{coverage}%"]
                }
            }
    ...
    def _execute_type3(self) -> 'CheckResult':
        ...
        details = [DetailItem(
            ...
            line_number=str(metadata['line']),
            file_path=str(metadata['file']),
            reason=f"DFT Stuck-At coverage: {coverage}%"
        )]
    ...
    def _execute_type4(self) -> 'CheckResult':
        ...
        if coverage >= expected_threshold:
            ...
            waive_groups = {
                "WAIVE01": {
                    "description": "DFT Stuck-At coverage waived",
                    "items": [f"ATCov%:{coverage}%"]
                }
            }
            ...
            error_groups = {
                "ERROR01": {
                    "description": "DFT Stuck-At coverage below waiver minimum",
                    "items": [f"ATCov%:{coverage}%"]
                }
            }
...
def run():
    """Execute the DFT Stuck-At coverage checker."""
    checker = DFTStuckatChecker()
