...
class Check_MIG_8_1_1_21(InputFileParserMixin, OutputBuilderMixin, WaiverHandlerMixin, BaseChecker):
    """
    ...
    def __init__(self):
        ...
        super().__init__(
            ...
            item_id="MIG-IMP-8-1-1-21",
            item_desc="Confirm dfi interface registers are placed close to DFI interface pins and make the DFI interface input/output clock/datapath shortest with minumum skew (If the project is dragonfly DDR5 and controller will use PHY phy_pclk_out, please don't build shortest clock tree for DFI interface register clocks. You can balance DFI interface clock tree with internal clock tree or a little larger than internal clock tree). Build separate clock tree to DFI regsiters. One is for input registers, the other is for output registers. And the input clock tree need to be balanced with output clock tree.(check the Note)"
        )
