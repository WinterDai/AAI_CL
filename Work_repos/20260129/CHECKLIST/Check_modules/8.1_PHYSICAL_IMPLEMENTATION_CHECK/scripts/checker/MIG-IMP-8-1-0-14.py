################################################################################
# Script Name: MIG-IMP-8-1-0-14.py
#
# Purpose:
#   Confirm clock latency (EFG paths) meet requirement for timing budget. Put the delay from DESKEW PLL to slice register CK(E) and pad_mem* (F+G) to comment collomn (target SS corner delay < 800ps).
#
# Logic:
...
class Check_MIG_8_1_0_14(InputFileParserMixin, OutputBuilderMixin, WaiverHandlerMixin, BaseChecker):
    """
    ...
    def __init__(self):
        ...
        super().__init__(
            ...
            item_id="MIG-IMP-8-1-0-14",
            item_desc="Confirm clock latency (EFG paths) meet requirement for timing budget. Put the delay from DESKEW PLL to slice register CK(E) and pad_mem* (F+G) to comment collomn (target SS corner delay < 800ps)."
        )
