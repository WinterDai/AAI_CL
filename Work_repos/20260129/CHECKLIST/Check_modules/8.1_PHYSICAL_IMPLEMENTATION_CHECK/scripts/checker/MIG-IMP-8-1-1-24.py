################################################################################
# Script Name: MIG-IMP-8-1-1-24.py
#
# Purpose:
#   Confirm Deskew/FRAC PLL(All power domains) Dcap meet the system requirement. (e.g: 100pf per deskew/fracN pll each power domain for 6400Mbps, Please check the Notes for decap number for each PLL power domain based on data rate). Follow the guidance in AMS integration guide specified in Notes.
#
# Logic:
...
class Check_MIG_8_1_1_24(InputFileParserMixin, OutputBuilderMixin, WaiverHandlerMixin, BaseChecker):
    """
    ...
    MISSING_REASON_TYPE2_3 = "TBD - e.g., 'Expected pattern not satisfied or missing'"
    
    # Waiver-related (Type 3/4 only)
    WAIVED_BASE_REASON = "TBD - why item was waived"
    ...
    def __init__(self):
        ...
        super().__init__(
            ...
            item_id="MIG-IMP-8-1-1-24",
            item_desc="Confirm Deskew/FRAC PLL(All power domains) Dcap meet the system requirement. (e.g: 100pf per deskew/fracN pll each power domain for 6400Mbps, Please check the Notes for decap number for each PLL power domain based on data rate). Follow the guidance in AMS integration guide specified in Notes."
        )
    ...
    def _execute_type3(self) -> CheckResult:
        ...
        # Fix: Use missing_items (not unwaived_items) and add found_items for complete output
        return self.build_complete_output(
            found_items=found_items,
