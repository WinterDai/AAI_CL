...
class BlackBoxChecker(BaseChecker):
    ...
    def _execute_type2(self, black_boxes: List[str]) -> CheckResult:
        ...
        return create_check_result(
            ...
            details=details,
            item_desc=self.item_desc,
            default_group_desc=f"Black boxes in design (found: {actual_value}, requirement: ≤{req_value})"
        )
    ...
    def _execute_type3(self, black_boxes: List[str]) -> CheckResult:
            ...
            for black_box in black_boxes:
                ...
                if black_box in waive_items_dict:
                    ...
                    details.append(DetailItem(
                        ...
                        line_number=metadata.get('line_number', ''),
                        file_path=metadata.get('file_path', ''),
                        reason=f"Black box found (requirement: ≤{req_value})"
                    ))
        ...
        return create_check_result(
            ...
            details=details,
            item_desc=self.item_desc,
            default_group_desc=f"Black boxes (found: {actual_value}, waived: {waived_count}, effective: {effective_value}, requirement: ≤{req_value})"
        )
