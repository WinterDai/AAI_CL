    ...
    for item in items:
        ...
        if executed:
            try:
                ...
                # Check if this is a configuration error template (not a real execution)
                is_config_error = "input_files is empty in YAML configuration" in content
                if is_config_error:
                    executed = False  # Mark as not executed
                    print(f"[DEBUG] {item}: Detected config error template, marking as not executed")
                else:
