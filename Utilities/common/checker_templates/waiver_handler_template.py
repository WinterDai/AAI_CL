...
class WaiverHandlerMixin:
    ...
    def apply_type1_type2_waiver(
        ...
        ⭐ NEW: Supports both string and callable (lambda) reasons!
        If reason is callable, returns a wrapped callable that adds tag dynamically.
        
        Args:
            is_pass_normal_mode: PASS/FAIL determination result in normal mode
            fail_reason: Reason description for FAIL status (string or callable)
            info_reason: Reason description for INFO status (string or callable)
        ...
        Returns:
            ...
            - reason: String or callable with waiver tag added if applicable
        
        Example - Type 1 with string:
        ...
        Example - Type 1 with lambda:
            is_pass, severity, reason = self.apply_type1_type2_waiver(
                is_pass_normal_mode=True,
                fail_reason=lambda item: f"{item['name']}: Missing",
                info_reason=lambda item: f"{item['name']}: Found"
            )
            # If waiver=0: (True, 'INFO', <lambda that adds [WAIVED_AS_INFO]>)
            # When called: reason(item) → "MyItem: Found[WAIVED_AS_INFO]"
        """
        if self.should_convert_fail_to_info():
            # waiver=0: Force PASS, convert to INFO
            reason = info_reason if info_reason else fail_reason
            
            # Support callable (lambda) reasons
            if callable(reason):
                # Return a wrapped lambda that adds tag when called
                if waiver_tag:
                    return True, 'INFO', lambda item: f"{reason(item)}{waiver_tag}"
                else:
                    return True, 'INFO', reason
            else:
                # String reason - add tag directly
